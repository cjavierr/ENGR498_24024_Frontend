...
const MergedDashboard = () => {
  ...
  const [selectedQuantitativeField, setSelectedQuantitativeField] = useState('');
  ...
  const onSaveField = () => {
    if (
      ...
      const fieldName = fieldNames[selectedQuantitativeField] || selectedQuantitativeField;
  ...
  const generateSpreadsheetData = (subcategories, periods) => {
    const data = [];
        ...
        <Form.Item label="Quantitative Field" rules={[{ required: true, message: 'Please select a quantitative field!' }]}>
          <Select value={selectedQuantitativeField} onChange={onQuantitativeFieldChange}>
            {Object.entries(fieldNames)
              .filter(([key]) => !quantitativeFields.some(field => field.fieldName === fieldNames[key]))
              .map(([key, value]) => (
